#include<bits/stdc++.h>
#define all(v) (v).begin(),(v).end()
#define pb push_back
#define ppb pop_back
#define mp make_pair
#define ri(x) scanf("%d",&(x))
#define ri2(x,y) scanf("%d %d",&(x),&(y))
#define ri3(x,y,z) scanf("%d %d %d",&(x),&(y),&(z))
#define rll(x) scanf("%lld",&(x))
#define rll2(x,y) scanf("%lld %lld",&(x),&(y))
#define rll3(x,y,z) scanf("%lld %lld %lld",&(x),&(y),&(z))
#define gc(x) ((x) = getchar())
using namespace::std;

const long double PI = acos(-1);
const long long MOD = 1000000000 +7;

typedef long long ll;
typedef pair<ll,ll> pll;
typedef pair<ll,pll> tll;
typedef pair<int,int> ii;
typedef pair<int,ii> iii;
typedef vector<int> vi;
typedef vector<ii> vii;
typedef vector<iii> viii;
typedef vector<ll> vll;
typedef vector<pll> vpll;
typedef vector<tll> vtll;
typedef vector<string> vs;
typedef set<int> si;
typedef set<ii> sii;
typedef set<iii> siii;

ll gcd(ll a, ll b){ return b==0?a:gcd(b,a%b);}

ll add(ll a, ll b, ll m = MOD){
	if(a >= m) a %= m;
	if(b >= m) b %= m;
	if(a < 0) a += m;
	if(b < 0) b += m;
	ll res = a+b;
	if(res >= m or res <= -m) res %= m;
	if(res < 0) res += m;
	return res;
}

ll mul(ll a, ll b, ll m = MOD){
	if(a >= m) a %= m;
	if(b >= m) b %= m;
	if(a < 0) a += m;
	if(b < 0) b += m;
	ll res = a*b;
	if(res >= m or res <= -m) res %= m;
	if(res < 0) res += m;
	return res;
}

ll pow_mod(ll a, ll b, ll m = MOD){
	ll res = 1LL;
	a = a%m;
	while(b){
		if(b&1) res = mul(res,a,m);
		b >>= 1;
		a = mul(a,a,m);
	}
	return res;
}

ll fastexp(ll a, ll b){
	ll res = 1LL;
	while(b){
		if(b&1) res = res*a;
		b >>= 1;
		a *= a;
	}
	return res;
}

int gcdExtendido(int a, int b, int *x, int *y){
	if(a == 0){
		*x = 0;
		*y = 1;
		return b;
	}
	int x1, y1;
	int gcd = gcdExtendido(b%a,a,&x1,&y1);
	
	*x = y1-(b/a)*x1;
	*y = x1;
	return gcd;
}

int modInverso(int a, int m){
	int x, y;
	int g = gcdExtendido(a,m,&x,&y);
	if(g!=1) return -1;
	else return (x%m + m)%m;
}

/****************************************
*************P*L*A*N*T*I*L*L*A************
*****************************************/

const int N = 100000 + 5;

mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());

int random(int l, int r){
	return uniform_int_distribution<int>(l, r)(rng);
}

struct node{
	int val;
	int cnt;
	int frec;
	int priority;
	node* left;
	node* right;
	node(int val, int frec = 1) : val(val), frec(frec) {
		cnt = frec;
		priority = random(0, 2e9);
		left = right = nullptr;
	}
};

int n;
int m;
int len;
node* T;
int x[N];
int c[N];
int r[N];
int y[N];
int d[N];
int ans[N];
map<int, int> id;
vector<int> Q[3 * N], I[3 * N];

void compress(){
	id.clear();
	len = 0;
	set<int> S;
	for(int i = 1; i <= n; i++){
		S.emplace(x[i]);
		S.emplace(x[i] + r[i] + 1);
	}
	for(int i = 1; i <= m; i++){
		S.emplace(y[i]);
	}
	for(auto x : S){
		id[x] = len;
		len += 1;
	}
	for(int i = 1; i <= n; i++){
		int l = id[x[i]];
		int R = id[x[i] + r[i] + 1];
		I[l].emplace_back(i);
		I[R].emplace_back(-i);
	}
	for(int i = 1; i <= m; i++){
		Q[id[y[i]]].emplace_back(i);
	}
}

int cnt(node* t){
	return t ? t -> cnt : 0;
}

void update(node* t){
	if(t) t -> cnt = cnt(t -> left) + cnt(t -> right) + t -> frec;
}

node* merge(node* l, node* r){
	if(l == nullptr) return r;
	if(r == nullptr) return l;
	if(l -> priority > r -> priority){
		l -> right = merge(l -> right, r);
		update(l);
		return l;
	}
	else{
		r -> left = merge(l, r -> left);
		update(r);
		return r;
	}
}

pair<node*, node*> split(node* t, int x){
	if(t == nullptr){
		return {nullptr, nullptr};
	}
	if(t -> val <= x){
		pair<node*, node*> p = split(t -> right, x);
		t -> right = p.first;
		update(t);
		return {t, p.second};
	}
	else{
		pair<node*, node*> p = split(t -> left, x);
		t -> left = p.second;
		update(t);
		return {p.first, t};
	}
}

node* insert(node* u, int x){
	pair<node*, node*> p1 = split(u, x);
	pair<node*, node*> p2 = split(p1.first, x - 1);
	int frec = cnt(p2.second) + 1;
	u = merge(p2.first, new node(x, frec));
	u = merge(u, p1.second);
	return u;
}

node* take(node* u, int x){
	pair<node*, node*> p1 = split(u, x);
	pair<node*, node*> p2 = split(p1.first, x - 1);
	int frec = cnt(p2.second) - 1;
	u = p2.first;
	if(frec > 0) u = merge(u, new node(x, frec));
	u = merge(u, p1.second);
	return u;
}

int getGeq(node* u, int x){
	int res = cnt(u);
	pair<node*, node*> p = split(u, x - 1);
	res -= cnt(p.first);
	u = merge(p.first, p.second);
	return res;
}

void solve(){
	T = nullptr;
	for(int i = 0; i < len; i++){
		for(int p : I[i]){
			if(p > 0){
				int key = c[p] - x[p];
				T = insert(T, key);
			}
			else{
				p = -p;
				int key = c[p] - x[p];
				T = take(T, key);
			}
		}
		for(int p : Q[i]){
			ans[p] = getGeq(T, d[p] - y[p]);
		}
	}
	for(int i = 0; i <= len; i++){
		Q[i].clear();
		I[i].clear();
	}
}

int main(){
	freopen("business.in", "r", stdin);
	int t;
	ri(t);
	while(t--){
		ri2(n, m);
		for(int i = 1; i <= n; i++){
			ri3(x[i], c[i], r[i]);
		}
		for(int i = 1; i <= m; i++){
			ri2(y[i], d[i]);
		}
		compress();
		solve();
		for(int i = 1; i <= m; i++){
			printf("%d%c", ans[i], " \n"[i == m]);
		}
	}
	return 0;
}
